...
class Country_manager(models.Manager):
    ...
    def get_random(self):
        """Pega um país aleatório. Usa o método 'ORDER BY RAND()' que é
        adequado para este propósito, mas lento em MySQL para tabelas maiores.
        Cuidado."""
        countries = self.order_by('?').first()
        if countries:
            return countries
        # Retornar um país específico (por exemplo, US) se a lista estiver vazia
        return self.filter(name_3char='US').first()
...
class Sitc4_cpy(models.Model):
    ...
    def __unicode__(self):
        return "CPY: %s.%s.%d" % (
            self.country.name, self.product.code, self.year)
...
class Sitc4_ccpy(models.Model):
    ...
    destination = models.ForeignKey(
        Country,
        related_name="sitc4_ccpys_destination")
    product = models.ForeignKey(Sitc4)
    export_value = models.FloatField(null=True)
    import_value = models.FloatField(null=True)
...
class Hs4_ccpy(models.Model):
    ...
    destination = models.ForeignKey(
        Country,
        related_name="hs4_ccpys_destination")
    product = models.ForeignKey(Hs4)
    export_value = models.FloatField(null=True)
    import_value = models.FloatField(null=True)
