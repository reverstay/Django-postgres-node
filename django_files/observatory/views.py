...
def explore(
    ...
    if was_redirected:
        ...
        if country1 == "zaf":
            warning = {
                "title": "Country Substitution", "text":
                    "In the Harmonized System (HS) classification, trade for Namibia, Republic of South Africa, Botswana, Lesotho e Swaziland é relatado sob 'South African Customs Union'."}
...
def explore_random(request):
    """Escolha um país aleatório e explore-o, para o link /explore no topo do template principal."""
    random_country = Country.objects.get_random()  # Corrigido para acessar o método do gerenciador de modelo
    random_country_code = random_country.name_3char.lower()
